...
class MyStaticMplCanvas(MyMplCanvas):
    ...
    @staticmethod
    def generateData(gs): # generate data to animate
            number = gs.num 
    ...
    def animationSteady(self,gs):
            proj3d.persp_transformation = self.orthogonal_proj
            data= self.generateData(gs)
            [[xmin,xmax,ymin,ymax,zmin,zmax],[az,el]]= self.getMinMax(data,gs)
...
class ApplicationWindow(QTabWidget):
    def __init__(self,gs):
        ...
        sc1.first_figure(gs)
        sc2.second_figure(gs)
        sc3.animationSteady(gs)
        sc4.animation(gs)
